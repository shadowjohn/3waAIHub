...
if (($_SERVER['REQUEST_METHOD'] ?? 'GET') === 'POST') {
    ...
    try {
        ...
        if ($action === 'save_roles') {
        ...
        } elseif ($action === 'toggle_station' || $action === 'refresh_station' || $action === 'delete_station') {
            ...
            if ($action === 'delete_station') {
                hub_cluster_delete_station($db, (int)$station['id']);
                $message = '子節點與其 Cluster 路由紀錄已刪除。';
...
<?php else: ?>
    ...
    <?php if ($routerEnabled): ?>
        ...
        <section>
            ...
            <?php if ($stationRows === []): ?>
                <div class="hub-empty-state">目前沒有已配對的子節點。</div>
            <?php else: ?>
