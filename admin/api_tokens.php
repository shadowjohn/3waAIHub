...
hub_admin_header('API Tokens', $user);
?>
...
<section class="panel">
    <h1>API Tokens</h1>
    <p><strong><?= hub_h($member['name']) ?></strong> <a class="button" href="api_members.php">返回 Members</a></p>
</section>
<section class="panel">
    <h2>建立 Token</h2>
    <form method="post">
...
<section class="panel">
    <h2>Tokens</h2>
    <table>
        <tr><th>ID</th><th>Name</th><th>Prefix</th><th>Enabled</th><th>Valid</th><th>Last used</th><th>操作</th></tr>
        <?php foreach ($tokens as $token): ?>
            ...
            <tr>
                ...
                <td class="<?= (int)$token['enabled'] === 1 && !$revoked ? 'ok' : 'bad' ?>"><?= $revoked ? '已 revoke' : ((int)$token['enabled'] === 1 ? '是' : '否') ?></td>
                ...
                <td class="actions">
                    <a class="button" href="api_token_permissions.php?token_id=<?= (int)$token['id'] ?>">Modes</a>
                    <a class="button" href="api_token_whitelist.php?token_id=<?= (int)$token['id'] ?>">IP</a>
                    <a class="button" href="api_usage.php?token_id=<?= (int)$token['id'] ?>">Usage</a>
                    <form method="post">
                        <input type="hidden" name="csrf_token" value="<?= hub_h(hub_csrf_token()) ?>">
                        <input type="hidden" name="member_id" value="<?= (int)$member['id'] ?>">
                        ...
                        <?php else: ?>
                            ...
                            <?php endif; ?>
                            <button class="danger" name="action" value="revoke" type="submit">Revoke</button>
                        <?php endif; ?>
                    </form>
                </td>
            </tr>
