...
<section class="panel">
    <h1><?= hub_h(__('帳號資料')) ?></h1>
    <p class="muted"><?= hub_h(__('可維護顯示名稱、Email 與公司 / 單位；角色與啟用狀態由系統管理員管理。')) ?></p>
...
<section class="panel">
    <form method="post">
        ...
        <div class="hub-meta">
            <div class="hub-meta-label"><?= hub_h(__('username')) ?></div><div class="hub-meta-value"><code><?= hub_h((string)$user['username']) ?></code></div>
            <div class="hub-meta-label"><?= hub_h(__('role')) ?></div><div class="hub-meta-value"><code><?= hub_h((string)$user['role']) ?></code></div>
        </div>
        <label><?= hub_h(__('顯示名稱')) ?></label>
        <input name="display_name" value="<?= hub_h((string)($user['display_name'] ?? '')) ?>">
        <label><?= hub_h(__('Email')) ?></label>
        <input name="email" type="email" value="<?= hub_h((string)($user['email'] ?? '')) ?>">
        <label><?= hub_h(__('公司 / 單位')) ?></label>
        <input name="company" value="<?= hub_h((string)($user['company'] ?? '')) ?>">
        <p><button class="primary" type="submit"><?= hub_h(__('儲存')) ?></button></p>
    </form>
</section>
