...
hub_admin_header(__('IP 白名單'), $user);
?>
<section class="panel">
    <h1><?= hub_h(__('IP 白名單')) ?></h1>
    <p class="muted"><?= hub_h(__('這裡管理的是自己的 Token IP 白名單。未設定任何規則時，Token 不限制來源 IP；有設定時必須符合其中一條。')) ?></p>
    <p class="muted">
        <?= hub_h(__('支援單一 IP 與 CIDR：')) ?><code>192.168.1.10</code>、<code>192.168.0.0/16</code>、<code>0.0.0.0/0</code>、<code>::/0</code>。
        <?= hub_h(__('不支援萬用字元：')) ?><code>192.168.*.*</code> <?= hub_h(__('請改用')) ?> <code>192.168.0.0/16</code>；<?= hub_h(__('全部開放最簡單是不要設定任何規則。')) ?>
    </p>
...
<section class="panel">
    ...
    <form method="post">
        ...
        <label><?= hub_h(__('備註')) ?></label>
        <input name="label">
        <p><button class="primary" type="submit"><?= hub_h(__('新增')) ?></button></p>
    </form>
</section>
...
<section class="panel">
    <h2><?= hub_h(__('目前規則')) ?></h2>
    <table>
        <thead><tr><th>IP / CIDR</th><th><?= hub_h(__('類型')) ?></th><th><?= hub_h(__('備註')) ?></th><th><?= hub_h(__('狀態')) ?></th><th><?= hub_h(__('操作')) ?></th></tr></thead>
        <tbody>
        <?php foreach ($rules as $rule): ?>
            <tr>
                ...
                <td><?= (int)$rule['enabled'] === 1 ? '<span class="ok">' . hub_h(__('啟用')) . '</span>' : '<span class="bad">' . hub_h(__('停用')) . '</span>' ?></td>
                <td>
                    <form method="post">
                        <input type="hidden" name="csrf_token" value="<?= hub_h(hub_csrf_token()) ?>">
                        ...
                        <button class="danger" type="submit"><?= hub_h(__('刪除')) ?></button>
                    </form>
                </td>
            </tr>
        <?php endforeach; ?>
...
<?php else: ?>
    <section class="panel"><div class="hub-empty-state"><?= hub_h(__('目前沒有可管理的 Token。')) ?></div></section>
<?php endif; ?>
