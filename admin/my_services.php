...
function hub_my_services_upload_label(array $service): string
{
    ...
    return (int)($gateway['max_upload_mb'] ?? 0) > 0 ? hub_i18n_text('需要 / 支援上傳') : hub_i18n_text('不需要');
}
...
hub_admin_header(hub_i18n_text('我的服務'), $user);
?>
<section class="panel">
    <h1><?= hub_h(hub_i18n_text('我的服務')) ?></h1>
    <p class="muted"><?= hub_h(hub_i18n_text('這裡只顯示你被授權可使用的 API mode。內部主機與 runtime 細節不會顯示。')) ?></p>
</section>

<section class="hub-card-grid">
<?php if ($services === []): ?>
    <div class="hub-empty-state"><?= hub_h(hub_i18n_text('目前尚未指派可用服務，請聯絡系統管理員。')) ?></div>
<?php endif; ?>
<?php foreach ($services as $service): ?>
    ...
    <article class="hub-card">
        ...
        <div class="hub-meta">
            ...
            <div class="hub-meta-label"><?= hub_h(hub_i18n_text('檔案上傳')) ?></div><div class="hub-meta-value"><?= hub_h(hub_my_services_upload_label($service)) ?></div>
            <div class="hub-meta-label">real_inference</div><div class="hub-meta-value"><?= hub_h((string)($pack['manifest']['runtime_level'] ?? '') >= 'L4' ? hub_i18n_text('支援或預留') : hub_i18n_text('未提供')) ?></div>
            <div class="hub-meta-label"><?= hub_h(hub_i18n_text('Token 權限')) ?></div><div class="hub-meta-value"><?= $hasTokenPermission ? '<span class="ok">' . hub_h(hub_i18n_text('已有可用 Token')) . '</span>' : '<span class="bad">' . hub_h(hub_i18n_text('尚無可用 Token')) . '</span>' ?></div>
        </div>
        <div class="hub-actions">
            <a class="button" href="playground.php?mode=<?= urlencode($mode) ?>"><?= hub_h(hub_i18n_text('到 API 測試場')) ?></a>
            <a class="button" href="../public_api_docs.php"><?= hub_h(hub_i18n_text('API 文件')) ?></a>
        </div>
    </article>
<?php endforeach; ?>
</section>
