...
<section class="panel">
    <h1><?= hub_h(__('記錄中心')) ?></h1>
    <p class="muted"><?= hub_h(__('集中查執行歷程、API 記錄、背景工作、服務記錄與系統記錄。IP 篩選的 GET link 一律使用 base64url。')) ?></p>
    <nav class="tabs" aria-label="<?= hub_h(__('記錄中心頁籤')) ?>">
        <?php foreach (hub_admin_record_tabs() as $tab => $label): ?>
            <a class="tab" aria-selected="<?= $activeTab === $tab ? 'true' : 'false' ?>" href="log_explorer.php?tab=<?= hub_h($tab) ?>"><?= hub_h(__($label)) ?></a>
        <?php endforeach; ?>
    </nav>
</section>
...
<section class="panel">
    ...
    <form method="get">
        ...
        <p><button class="primary" type="submit"><?= hub_h(__('查詢')) ?></button> <a class="button" href="log_explorer.php?tab=runs"><?= hub_h(__('清除')) ?></a></p>
    </form>
</section>
<section class="panel">
    <div class="hub-section-title">
        <h2><?= hub_h(__('執行歷程')) ?></h2>
        <span class="muted"><?= count($runs) ?> <?= hub_h(__('筆')) ?></span>
    </div>
    <?php if ($runs === []): ?>
        <div class="hub-empty-state"><?= hub_h(__('目前沒有 Runtime 執行紀錄。')) ?></div>
    <?php else: ?>
        <div class="record-table-wrap">
            <table>
                <thead><tr>
                    <th>Run ID</th><th>Pack</th><th>Job</th><th><?= hub_h(__('狀態')) ?></th><th><?= hub_h(__('開始時間')) ?></th><th><?= hub_h(__('耗時')) ?></th><th><?= hub_h(__('RAM 峰值')) ?></th><th><?= hub_h(__('VRAM 峰值')) ?></th><th><?= hub_h(__('結束碼')) ?></th><th><?= hub_h(__('操作')) ?></th>
                </tr></thead>
                <tbody>
                <?php foreach ($runs as $run): ?>
                    <tr>
                        ...
                        <td><a class="button" href="runtime_run.php?id=<?= urlencode((string)$run['run_id']) ?>"><?= hub_h(__('查看詳情')) ?></a></td>
                    </tr>
                <?php endforeach; ?>
                </tbody>
            </table>
...
<section class="panel">
    ...
    <form method="post">
        ...
        <label><?= hub_h(__('結果')) ?></label>
        <select name="ok">
            <option value=""><?= hub_h(__('全部')) ?></option>
            <option value="1"<?= $apiFilters['ok'] === '1' ? ' selected' : '' ?>><?= hub_h(__('成功')) ?></option>
            <option value="0"<?= $apiFilters['ok'] === '0' ? ' selected' : '' ?>><?= hub_h(__('失敗')) ?></option>
        </select>
        <label>HTTP <?= hub_h(__('狀態碼')) ?></label>
        <input name="status_code" value="<?= hub_h($apiFilters['status_code']) ?>">
        <label><?= hub_h(__('錯誤碼')) ?></label>
        <input name="error_code" value="<?= hub_h($apiFilters['error_code']) ?>">
        <label>HTTP <?= hub_h(__('方法')) ?></label>
        <input name="method" value="<?= hub_h($apiFilters['method']) ?>">
        <label>Request ID</label>
        <input name="request_id" value="<?= hub_h($apiFilters['request_id']) ?>">
        <label><?= hub_h(__('關鍵字')) ?></label>
        <input name="keyword" value="<?= hub_h($apiFilters['keyword']) ?>" placeholder="request_uri / reason / user_agent">
        <p><button class="primary" type="submit"><?= hub_h(__('查詢')) ?></button> <a class="button" href="log_explorer.php?tab=api"><?= hub_h(__('清除')) ?></a></p>
    </form>
</section>
<section class="panel">
    <h2><?= hub_h(__('最近 24 小時')) ?></h2>
    <div class="record-stats">
        <?php foreach (['failed_ips' => __('失敗最多 IP'), 'error_codes' => __('常見錯誤碼'), 'unknown_modes' => __('未知 Mode'), 'denied_ips' => __('被拒絕 IP')] as $kind => $title): ?>
            <div>
                <h3><?= hub_h($title) ?></h3>
                <table>
...
<section class="panel">
    <h2><?= hub_h(__('記錄列表')) ?></h2>
    <p class="muted"><?= hub_h(__('共')) ?> <?= (int)$total ?> <?= hub_h(__('筆，第')) ?> <?= (int)$page ?> <?= hub_h(__('頁，每頁')) ?> <?= (int)$limit ?> <?= hub_h(__('筆。')) ?></p>
    <div class="record-table-wrap">
        <table>
            <thead><tr>
                <th><?= hub_h(__('時間')) ?></th><th>IP</th><th><?= hub_h(__('會員')) ?></th><th>Token</th><th>Mode</th><th><?= hub_h(__('服務')) ?></th><th>HTTP <?= hub_h(__('方法')) ?></th><th>HTTP <?= hub_h(__('狀態')) ?></th><th><?= hub_h(__('結果')) ?></th><th><?= hub_h(__('錯誤')) ?></th><th><?= hub_h(__('原因')) ?></th><th><?= hub_h(__('耗時')) ?> ms</th><th><?= hub_h(__('容量')) ?></th><th>Request ID</th><th>UA</th>
            </tr></thead>
            <tbody>
            <?php foreach ($logs as $log): ?>
                <tr>
                    ...
                    <td class="<?= (int)$log['ok'] === 1 ? 'ok' : 'bad' ?>"><?= hub_h((int)$log['ok'] === 1 ? __('成功') : __('失敗')) ?></td>
    ...
    <p>
        <?php if ($page > 1): ?>
            <a class="button" href="log_explorer.php?<?= hub_h($baseQuery . '&' . 'page=' . ($page - 1)) ?>"><?= hub_h(__('上一頁')) ?></a>
        <?php endif; ?>
        <?php if (($page * $limit) < $total): ?>
            <a class="button" href="log_explorer.php?<?= hub_h($baseQuery . '&' . 'page=' . ($page + 1)) ?>"><?= hub_h(__('下一頁')) ?></a>
        <?php endif; ?>
    </p>
</section>
...
<section class="panel">
    <h2><?= hub_h(__('背景工作')) ?></h2>
    <p class="muted"><?= hub_h(__('查 command_jobs 排程與執行結果。stdout_tail / stderr_tail 只顯示最後 6000 bytes。')) ?></p>
    <form method="get">
        ...
        <select name="status">
            ...
            <?php foreach (hub_admin_record_job_statuses() as $status): ?>
                <option value="<?= hub_h($status) ?>"<?= $jobFilters['status'] === $status ? ' selected' : '' ?>><?= hub_h(__(hub_command_status_label($status))) ?> / <?= hub_h($status) ?></option>
            <?php endforeach; ?>
        </select>
        <label><?= hub_h(__('動作')) ?></label>
        <select name="action">
            <option value=""><?= hub_h(__('全部')) ?></option>
            <?php foreach (hub_allowed_job_actions() as $action): ?>
                <option value="<?= hub_h($action) ?>"<?= $jobFilters['action'] === $action ? ' selected' : '' ?>><?= hub_h(__(hub_command_action_label($action))) ?> / <?= hub_h($action) ?></option>
            <?php endforeach; ?>
        </select>
        <label><?= hub_h(__('服務')) ?></label>
        <select name="service_id">
        ...
        <p><button class="primary" type="submit"><?= hub_h(__('查詢')) ?></button> <a class="button" href="log_explorer.php?tab=jobs"><?= hub_h(__('清除')) ?></a></p>
    </form>
</section>
<section class="panel">
    <div class="hub-section-title">
        <h2><?= hub_h(__('背景工作列表')) ?></h2>
        <span class="muted"><?= count($jobLogs) ?> <?= hub_h(__('筆')) ?></span>
    </div>
    <div class="record-table-wrap">
        <table>
            <thead><tr>
                <th><?= hub_h(__('工作 ID')) ?></th><th><?= hub_h(__('建立時間')) ?></th><th><?= hub_h(__('更新時間')) ?></th><th><?= hub_h(__('動作')) ?></th><th><?= hub_h(__('服務')) ?></th><th><?= hub_h(__('狀態')) ?></th><th><?= hub_h(__('進度')) ?></th><th><?= hub_h(__('階段')) ?></th><th><?= hub_h(__('結束碼')) ?></th><th><?= hub_h(__('請求來源')) ?></th><th><?= hub_h(__('錯誤訊息')) ?></th><th>stdout_tail / stderr_tail</th>
            </tr></thead>
            <tbody>
            <?php foreach ($jobLogs as $job): ?>
                ...
                <tr>
                    ...
                    <td><?= hub_h(__(hub_command_action_label((string)$job['action']))) ?><br><code><?= hub_h((string)$job['action']) ?></code></td>
                    <td><?= hub_h((string)($job['service_name'] ?? '')) ?><br><code><?= hub_h((string)($job['service_key'] ?? '')) ?></code></td>
                    <td class="<?= hub_h(hub_command_status_class((string)$job['status'])) ?>"><?= hub_h(__(hub_command_status_label((string)$job['status']))) ?><br><code><?= hub_h((string)$job['status']) ?></code></td>
                    ...
                    <td>IP: <code><?= hub_h((string)($job['requested_ip'] ?? '')) ?></code><br><?= hub_h(__('請求人')) ?>: <?= hub_h((string)($job['requested_by_username'] ?? $job['requested_by'] ?? '')) ?></td>
...
<section class="panel">
    ...
    <form method="get">
        ...
        <p><button class="primary" type="submit"><?= hub_h(__('查詢')) ?></button> <a class="button" href="log_explorer.php?tab=service"><?= hub_h(__('清除')) ?></a></p>
    </form>
</section>
<section class="panel">
    <div class="hub-section-title">
        <h2><?= hub_h(__('服務記錄列表')) ?></h2>
        <span class="muted"><?= count($serviceLogs) ?> <?= hub_h(__('筆')) ?></span>
    </div>
    <div class="record-table-wrap">
        <table>
            <thead><tr><th><?= hub_h(__('時間')) ?></th><th><?= hub_h(__('服務')) ?></th><th><?= hub_h(__('動作')) ?></th><th><?= hub_h(__('結束碼')) ?></th><th><?= hub_h(__('輸出')) ?></th></tr></thead>
            <tbody>
...
<section class="panel">
    <div class="hub-section-title">
        <h2><?= hub_h(__('系統記錄')) ?></h2>
        <span class="muted"><?= count($systemLogs) ?> <?= hub_h(__('筆')) ?></span>
    </div>
    <div class="record-table-wrap">
        <table>
            <thead><tr><th><?= hub_h(__('時間')) ?></th><th><?= hub_h(__('使用者')) ?></th><th><?= hub_h(__('動作')) ?></th><th><?= hub_h(__('內容')) ?></th></tr></thead>
            <tbody>
