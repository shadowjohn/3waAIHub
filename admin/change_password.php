...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($newPassword !== $confirmPassword) {
        $error = hub_i18n_text('新密碼與確認密碼不一致。');
    } else {
...
hub_admin_header(hub_i18n_text('變更密碼'), $user);
?>
<section class="panel">
    <h1><?= hub_h(hub_i18n_text('變更密碼')) ?></h1>
...
<section class="panel">
    <form method="post">
        ...
        <label><?= hub_h(hub_i18n_text('目前密碼')) ?></label>
        <input name="current_password" type="password" autocomplete="current-password" required>
        <label><?= hub_h(hub_i18n_text('新密碼')) ?></label>
        <input name="new_password" type="password" autocomplete="new-password" required>
        <label><?= hub_h(hub_i18n_text('確認新密碼')) ?></label>
        <input name="confirm_password" type="password" autocomplete="new-password" required>
        <p><button class="primary" type="submit"><?= hub_h(hub_i18n_text('更新密碼')) ?></button></p>
    </form>
</section>
