...
function hub_playground_facebook_action(PDO $db, string $action, string $token, array $post): array
{
    ...
    return hub_playground_guard_result(['error' => 'invalid_action', 'message' => hub_i18n_text('無效的操作。')]);
}
...
function hub_playground_facebook_request_fields_html(array $profiles, array $post): string
{
    ...
    ob_start();
    ?>
    <label><?= hub_h(hub_i18n_text('Facebook 登入 Profile')) ?> profile_id</label>
    <select name="facebook_profile_id">
        <option value=""><?= hub_h(hub_i18n_text('公開頁面，不使用登入 Profile')) ?></option>
    ...
    <label><?= hub_h(hub_i18n_text('目標粉絲專頁／社團')) ?></label>
    <textarea name="facebook_targets" rows="6" required><?= hub_h($targets) ?></textarea>
    <p class="muted"><?= hub_h(hub_i18n_text('每行一個 Facebook URL；一次最多 30 個目標。')) ?></p>
    <label><?= hub_h(hub_i18n_text('每個目標近期文章數')) ?> limit_per_target</label>
...
function hub_playground_facebook_management_html(array $profiles): string
{
    ...
    ?>
    <hr>
    <h3><?= hub_h(hub_i18n_text('Facebook 登入 Profile')) ?></h3>
    <p class="muted"><?= hub_h(hub_i18n_text('登入狀態只保存在本機節點；2FA 或 CAPTCHA 請在短效登入頁手動完成。')) ?></p>
    <form method="post">
        ...
        <label><?= hub_h(hub_i18n_text('Profile 名稱')) ?></label>
        <input name="facebook_display_name" maxlength="120" required>
        <label><?= hub_h(hub_i18n_text('登入方式')) ?></label>
        <select name="facebook_login_method">
            <option value="browser"><?= hub_h(hub_i18n_text('瀏覽器手動登入')) ?></option>
            <option value="password"><?= hub_h(hub_i18n_text('帳號密碼輔助登入')) ?></option>
        </select>
        <label><?= hub_h(hub_i18n_text('Facebook 帳號（僅密碼輔助登入）')) ?></label>
        <input name="facebook_username" autocomplete="off">
        <label><?= hub_h(hub_i18n_text('Facebook 密碼（不保存）')) ?></label>
        <input name="facebook_password" type="password" autocomplete="new-password">
        <div class="hub-actions"><button class="primary" type="submit"><?= hub_h(hub_i18n_text('開啟短效登入')) ?></button></div>
    </form>
    <form method="post">
        <input type="hidden" name="csrf_token" value="<?= hub_h(hub_csrf_token()) ?>">
        <input type="hidden" name="mode" value="facebook_crawl">
        <label>Bearer Token</label>
        <input name="bearer_token" type="password" placeholder="&lt;TOKEN&gt;" autocomplete="off" required>
        <label><?= hub_h(hub_i18n_text('已管理 Profile')) ?></label>
        <select name="facebook_profile_manage_id" required>
            <option value=""><?= hub_h(hub_i18n_text('請先載入 Profile')) ?></option>
            <?php foreach ($profiles as $profile): ?>
                <option value="<?= hub_h((string)$profile['profile_id']) ?>"><?= hub_h((string)$profile['display_name']) ?> / <?= hub_h((string)$profile['state']) ?></option>
            <?php endforeach; ?>
        </select>
        <div class="hub-actions">
            <button type="submit" name="action" value="facebook_profile_status"><?= hub_h(hub_i18n_text('查看狀態')) ?></button>
            <button type="submit" name="action" value="facebook_profile_reauth"><?= hub_h(hub_i18n_text('重新登入')) ?></button>
            <button class="danger" type="submit" name="action" value="facebook_profile_delete"><?= hub_h(hub_i18n_text('刪除 Profile')) ?></button>
        </div>
    </form>
    <hr>
    <h3><?= hub_h(hub_i18n_text('Dataset 預覽')) ?></h3>
    <form method="post">
        <input type="hidden" name="csrf_token" value="<?= hub_h(hub_csrf_token()) ?>">
        <input type="hidden" name="mode" value="facebook_crawl">
        <label>Bearer Token</label>
        <input name="bearer_token" type="password" placeholder="&lt;TOKEN&gt;" autocomplete="off" required>
        <label>task_id <?= hub_h(hub_i18n_text('（留空取最新可用 Dataset）')) ?></label>
        <input name="facebook_task_id" type="number" min="1">
        <div class="hub-actions">
            <button type="submit" name="action" value="facebook_run_last"><?= hub_h(hub_i18n_text('最後一次 Run')) ?></button>
            <button type="submit" name="action" value="facebook_dataset_preview"><?= hub_h(hub_i18n_text('Dataset 預覽')) ?></button>
        </div>
    </form>
    <?php
    return (string)ob_get_clean();
...
function hub_playground_facebook_result_html(?array $result): string
{
    ...
    <div data-facebook-task-links>
        <h3><?= hub_h(hub_i18n_text('任務連結')) ?></h3>
        <div class="hub-actions">
    ...
    <div data-facebook-dataset-preview>
        ...
        <?php else: ?>
            <p class="muted"><?= hub_h(hub_i18n_text('完成背景任務後，可用 task_id 預覽前 10 筆資料。')) ?></p>
        <?php endif; ?>
    </div>
    <?php
    return (string)ob_get_clean();
