...
2. Build the pinned Pack image, then run its read-only GPU smoke before downloading. On native Linux use `docker run --rm --pull never --gpus all --entrypoint /usr/bin/python3 3waaihub/vlm-manual-vision:0.1.0 /app/gpu_smoke.py`; on Windows run the same Docker command inside the configured WSL distribution. Confirm CUDA, driver, device, and free VRAM. This probe does not stop or start any service.
...
The model snapshot, cache, verified marker, acceptance JSON, immutable revision, and `HF_TOKEN` are node-private and must never enter Git, tickets, or public logs. Backup retention for these node assets is a separate operator policy review; do not improvise it during recovery.
...
`test_cluster_router.php`, `test_command_queue.php`, and `test_public_api_docs.php` are loaded by the control-plane runner and are not standalone scripts. The direct FastAPI endpoint tests require the Pack-pinned Python dependencies from `service/requirements.txt`; a host FastAPI installation is not the deployment runtime. Real provision, CUDA acceptance, and Router publication remain authorized deployment steps after merge.
