...
hub_test('PhaseP-1 generated compose has fixed image tag and start/build commands are split', function (): void {
    ...
    hub_test_assert(hub_service_image_tag($asr) === '3waaihub/whisper-asr:0.1.2', 'Whisper image checks must match its generated compose image');
...
SH
    ...
    try {
        ...
        hub_test_assert(($refreshed['pack_version'] ?? '') === '0.1.9' && str_ends_with($expectedTag, ':0.1.9'), 'restart must refresh the stored VoxCPM2 version before resolving its image');
