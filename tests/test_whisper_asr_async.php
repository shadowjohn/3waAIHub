...
hub_test('Whisper ASR declares the fixed GPU transcription Pack job', function (): void {
    ...
    hub_test_assert(is_file(HUB_ROOT . '/packs/whisper-asr/jobs/speech_transcribe.sh') && is_file(hub_test_whisper_async_runner()), 'Whisper Pack runner assets missing');
});
...
def run(request, token=None):
    with tempfile.TemporaryDirectory() as directory:
        ...
        (input_dir / "runner_config.json").write_text(json.dumps({"model": {"model": "large-v3"}}), encoding="utf-8")
...
result = run(base)
assert result["transcript"]["text"] == "hello" and loads == [("asr", "large-v3")]
loads.clear()
run(base | {"word_timestamps": True})
assert loads == [("asr", "large-v3"), ("align", "en")]
loads.clear()
try:
