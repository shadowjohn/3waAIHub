...
assert_no_mutations() {
    local log=$1 output
    output=$(<"$log")
    assert_not_contains 'network create' "$output"
    assert_not_contains 'iptables -N' "$output"
    assert_not_contains 'iptables -A' "$output"
    assert_not_contains 'iptables -I' "$output"
}
...
network:inspect)
    if [ "${MOCK_NETWORK_EXISTS:-0}" = 1 ] || [ -f "$MOCK_STATE/network-created" ]; then
        printf '%s\n' "${MOCK_NETWORK_STATE:-bridge|1|172.31.240.0/24|false}"
        exit 0
    fi
    exit 1
    ;;
...
-S)
    position="${MOCK_JUMP_POSITION:-front}"
    [ -f "$MOCK_STATE/jump-front" ] && position=front
    printf '%s\n' '-N DOCKER-USER'
    case "$position" in
...
if ! ready=$(run "$log" MOCK_NETWORK_EXISTS=1 "$script" --check); then
    fail '--check must accept the exact bridge network and first DOCKER-USER jump'
fi
[ "$ready" = 'capture_egress=ready' ] || fail '--check must print only the ready status'
check_log=$(<"$log")
assert_contains 'docker network inspect -f {{.Driver}}|{{len .IPAM.Config}}|{{(index .IPAM.Config 0).Subnet}}|{{.EnableIPv6}} aihub-capture-egress' "$check_log"
assert_contains 'iptables -S DOCKER-USER' "$check_log"
...
if run "$log" MOCK_NETWORK_EXISTS=1 MOCK_NETWORK_STATE='bridge|1|172.31.241.0/24|false' "$script" --check > "$tmp/check-wrong-subnet.out"; then
...
if run "$log" MOCK_NETWORK_EXISTS=1 MOCK_NETWORK_STATE='bridge|1|172.31.240.0/24|true' "$script" --check > "$tmp/check-ipv6.out"; then
...
log="$tmp/check-wrong-driver.log"
if run "$log" MOCK_NETWORK_EXISTS=1 MOCK_NETWORK_STATE='overlay|1|172.31.240.0/24|false' "$script" --check > "$tmp/check-wrong-driver.out"; then
    fail '--check must fail for a non-bridge Docker network'
fi
[ ! -s "$tmp/check-wrong-driver.out" ] || fail '--check must not report ready for a non-bridge network'
assert_no_mutations "$log"

log="$tmp/check-extra-ipam.log"
if run "$log" MOCK_NETWORK_EXISTS=1 MOCK_NETWORK_STATE='bridge|2|172.31.240.0/24|false' "$script" --check > "$tmp/check-extra-ipam.out"; then
    fail '--check must fail for additional IPAM configurations'
fi
[ ! -s "$tmp/check-extra-ipam.out" ] || fail '--check must not report ready for additional IPAM configurations'
assert_no_mutations "$log"

log="$tmp/check-shadowed-jump.log"
if run "$log" MOCK_NETWORK_EXISTS=1 MOCK_JUMP_POSITION=after-accept "$script" --check > "$tmp/check-shadowed-jump.out"; then
    fail '--check must fail when an earlier DOCKER-USER rule bypasses the scoped jump'
fi
[ ! -s "$tmp/check-shadowed-jump.out" ] || fail '--check must not report ready for a shadowed jump'
assert_no_mutations "$log"

log="$tmp/install-wrong-network.log"
if run "$log" MOCK_NETWORK_EXISTS=1 MOCK_NETWORK_STATE='overlay|1|172.31.240.0/24|false' "$script"; then
    fail 'installer must reject an existing network with the wrong shape'
fi
assert_no_mutations "$log"

log="$tmp/install-priority-jump.log"
run "$log" MOCK_NETWORK_EXISTS=1 MOCK_CHAIN_EXISTS=0 MOCK_IPTABLES_CHECK_EXIT=0 MOCK_JUMP_POSITION=after-accept "$script"
priority_log=$(<"$log")
assert_contains 'iptables -I DOCKER-USER 1 -s 172.31.240.0/24 -j AIHUB_CAPTURE_EGRESS' "$priority_log"
if ! priority_ready=$(run "$log" MOCK_NETWORK_EXISTS=1 "$script" --check); then
    fail 'installer must leave the scoped jump at the effective first position'
fi
[ "$priority_ready" = 'capture_egress=ready' ] || fail 'priority repair must pass --check'
...
assert_not_contains 'iptables -A DOCKER-USER -s 172.31.240.0/24 -j AIHUB_CAPTURE_EGRESS' "$rules_log"
...
assert_not_contains 'iptables -I' "$idempotent_log"
...
cron_dropin="$root/deploy/systemd/cron.service.d/aihub-capture-egress.conf"
[ -f "$cron_dropin" ] || fail 'cron must require the capture egress service'
dropin=$(<"$cron_dropin")
assert_contains 'Requires=aihub-capture-egress.service' "$dropin"
assert_contains 'After=aihub-capture-egress.service' "$dropin"
assert_contains 'PartOf=aihub-capture-egress.service' "$dropin"

ci=$(<"$root/.github/workflows/ci.yml")
assert_contains 'bash tests/test_capture_egress_network.sh' "$ci"
