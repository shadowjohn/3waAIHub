...
hub_test('release builder creates a hash-verified private deployment artifact', function (): void {
    ...
    $bytecodeCache = HUB_ROOT . '/scripts/__pycache__';
    $bytecodeFixture = $bytecodeCache . '/release_artifact_fixture_' . bin2hex(random_bytes(6)) . '.pyc';
    $createdBytecodeCache = false;

    try {
        if (!is_dir($bytecodeCache)) {
            hub_test_assert(mkdir($bytecodeCache, 0775), 'cannot create Python bytecode cache fixture directory');
            $createdBytecodeCache = true;
        }
        hub_test_assert(file_put_contents($bytecodeFixture, 'generated bytecode fixture', LOCK_EX) !== false, 'cannot create Python bytecode fixture');
        ...
        if (is_file($bytecodeFixture)) {
            unlink($bytecodeFixture);
        }
        if ($createdBytecodeCache && is_dir($bytecodeCache)) {
            rmdir($bytecodeCache);
        }
    }
});
