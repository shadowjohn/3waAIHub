...
hub_test('i18n sqlite table helper and language cookie contract work', function (): void {
    ...
    hub_test_assert(hub_i18n_text('控制台', 'zh_TW') === '控制台', 'zh_TW should return source text without lookup');

    $db->prepare('INSERT INTO i18n (title, lang, trans) VALUES (:title, :lang, :trans)')
        ->execute([':title' => '控制台', ':lang' => 'en', ':trans' => 'Dashboard']);
    hub_test_assert(hub_i18n_text('控制台', 'en') === 'Dashboard', 'i18n helper should read latest translation');
    hub_test_assert(hub_i18n_text('控制台', 'bad_lang') === '控制台', 'invalid language should fall back to zh_TW');
...
hub_test('i18n database text remains escaped at HTML rendering boundaries', function (): void {
    $db = hub_test_reset_db();
    $payload = '<img src=x onerror=alert(1)>';
    $db->prepare('INSERT INTO i18n (title, lang, trans) VALUES (:title, :lang, :trans)')
        ->execute([':title' => '安全翻譯測試', ':lang' => 'en', ':trans' => $payload]);

    hub_test_assert(hub_i18n_plain_text(' plain text ') === 'plain text', 'i18n plain-text helper must preserve regular text');
    hub_test_assert(hub_i18n_plain_text($payload) === '', 'i18n plain-text helper must reject HTML tag delimiters');
    hub_i18n_google_circuit_state(true);
    try {
        $translation = hub_i18n_text('安全翻譯測試', 'en');
    } finally {
        hub_i18n_google_circuit_state(false);
    }
    hub_test_assert($translation === '安全翻譯測試', 'i18n lookup must reject a stored HTML payload and use the plain source title');
    hub_test_assert(hub_h($translation) === '安全翻譯測試', 'HTML rendering must receive a safe plain-text translation');
});
...
hub_test('i18n seed imports without overwriting local translations', function (): void {
    ...
    hub_test_assert(hub_i18n_text('控制台', 'en') === 'Dashboard', 'seed translation missing');
    ...
    hub_test_assert(hub_i18n_text('控制台', 'en') === 'Local Dashboard', 'local translation must win');
...
hub_test('i18n seed round-trip keeps keyed Chinese rows and rejects malformed input', function (): void {
    ...
    file_put_contents($seed, json_encode([
        ...
        ['title' => 'pack.unsafe.description', 'lang' => 'en', 'trans' => '<script>alert(1)</script>'],
        'not-an-array',
    ], JSON_UNESCAPED_UNICODE));
...
hub_test('admin i18n validation allows only keyed zh_TW titles', function (): void {
    ...
    $html = hub_test_admin_i18n_post(['title' => 'Unsafe translation', 'lang' => 'en', 'trans' => '<script>alert(1)</script>']);
    hub_test_assert(str_contains($html, '請填寫標題、有效語系與翻譯內容。'), 'admin must reject an HTML translation payload');
    hub_test_assert((int)$db->query("SELECT COUNT(*) FROM i18n WHERE title = 'Unsafe translation'")->fetchColumn() === 0, 'admin stored an HTML translation payload');
});
...
hub_test('admin i18n maintenance tab and language selectors are present', function (): void {
    ...
    hub_test_assert(str_contains($layout, "hub_i18n_text('控制台')"), 'admin nav labels must call hub_i18n_text()');
    ...
    hub_test_assert(str_contains($home, "hub_i18n_text('公開 API 文件')"), 'home page links must call hub_i18n_text()');
});
...
hub_test('admin dashboard primary labels use i18n helper', function (): void {
    ...
    foreach ([
        ...
        "hub_i18n_text('總覽摘要')",
        "hub_i18n_text('服務總數')",
        "hub_i18n_text('API 24h 呼叫數')",
        "hub_i18n_text('平台能力矩陣')",
        "hub_i18n_text('最近背景工作')",
        "hub_i18n_text('待處理項')",
        "hub_i18n_text('服務管理')",
    ] as $needle) {
        hub_test_assert(str_contains($dashboard, $needle), 'dashboard label must call hub_i18n_text(): ' . $needle);
    }
});
...
hub_test('8/7 admin redesign routes visible labels through i18n without changing technical contracts', function (): void {
    ...
    foreach (['控制台', '安裝套件', '客戶管理', 'API 金鑰', 'Cluster 管理', '測試中心', '記錄中心', '系統環境', '系統設定'] as $label) {
        hub_test_assert(
            str_contains($layout, "hub_i18n_text('{$label}')"),
            'admin navigation label must call hub_i18n_text(): ' . $label
        );
    }
    ...
    hub_test_assert(
        str_contains($marketSource, 'hub_h(hub_i18n_text((string)$categoryLabel))'),
        'Market category labels must pass through hub_i18n_text()'
    );
    foreach (['套件市集', '已安裝服務', '安裝套件工作區'] as $label) {
        hub_test_assert(str_contains($marketSource, "hub_i18n_text('{$label}')"), 'Market workspace label must call hub_i18n_text(): ' . $label);
    }
    ...
    hub_test_assert(
        str_contains($recordSource, 'hub_h(hub_i18n_text($label))'),
        'Record Center tab labels must pass through hub_i18n_text()'
    );
    ...
    foreach ([
        ...
        hub_test_assert(str_contains($dashboard, "hub_i18n_text('{$label}')"), 'Dashboard state label must call hub_i18n_text(): ' . $label);
    }

    $environment = (string)file_get_contents(HUB_ROOT . '/admin/environment.php');
    foreach (['目前版本', '工作樹狀態', '最後檢查時間', '項次', '節點類型', 'Pack數', '狀況'] as $label) {
        hub_test_assert(str_contains($environment, "hub_i18n_text('{$label}')"), 'release label must call hub_i18n_text(): ' . $label);
    }

    $settings = (string)file_get_contents(HUB_ROOT . '/admin/settings.php');
    foreach (['上傳 Logo', '上傳並套用', '恢復預設'] as $label) {
        hub_test_assert(str_contains($settings, "hub_i18n_text('{$label}')"), 'branding action must call hub_i18n_text(): ' . $label);
    }

    foreach (['排隊中', '啟動中', '執行中', '異常'] as $label) {
        hub_test_assert(str_contains($marketSource, "hub_i18n_text('{$label}')"), 'service state must call hub_i18n_text(): ' . $label);
    }
    ...
    foreach ([
        ...
        hub_test_assert(str_contains($source, "hub_i18n_text('Legacy debug 頁面')"), $legacyPage . ' notice must call hub_i18n_text()');
        hub_test_assert(
            str_contains($source, "hub_i18n_text('此頁已退出主選單，正式操作請使用"),
            $legacyPage . ' retirement notice must call hub_i18n_text()'
        );
    }
    ...
    foreach ([
        ...
        preg_match_all('/hub_i18n_text\(\s*([\'"])((?:\\\\.|(?!\1).)*)\1\s*\)/us', $source, $matches);
...
hub_test('dashboard and playground script translations use HTML-safe JSON', function (): void {
    ...
    foreach ([
        "hub_json_encode(hub_i18n_text('顯示 token'), JSON_UNESCAPED_UNICODE)",
        "hub_json_encode(hub_i18n_text('隱藏 token'), JSON_UNESCAPED_UNICODE)",
        "hub_json_encode(hub_i18n_text('請手動複製。'), JSON_UNESCAPED_UNICODE)",
        "hub_json_encode(hub_i18n_text('已複製。'), JSON_UNESCAPED_UNICODE)",
    ] as $needle) {
...
hub_test('admin models page primary labels use i18n helper', function (): void {
    ...
    foreach ([
        "hub_i18n_text('模型倉庫')",
        "hub_i18n_text('模型根目錄概覽')",
        "hub_i18n_text('磁碟空間')",
        "hub_i18n_text('連結服務')",
        "hub_i18n_text('常見模型子目錄')",
        "hub_i18n_text('建立子目錄')",
        "hub_i18n_text('模型檔案清單')",
    ] as $needle) {
        hub_test_assert(str_contains($models, $needle), 'models page label must call hub_i18n_text(): ' . $needle);
    }
});
...
hub_test('admin settings page primary labels use i18n helper', function (): void {
    ...
    foreach ([
        ...
        "hub_i18n_text('多國語系')",
        "hub_i18n_text('儲存與模型')",
        "hub_i18n_text('API 與安全')",
        "hub_i18n_text('Docker 與背景工作')",
        "hub_i18n_text('維護與保留')",
        "hub_i18n_text('帳號密碼')",
    ] as $needle) {
        hub_test_assert(str_contains($settings, $needle), 'settings page label must call hub_i18n_text(): ' . $needle);
    }
});
...
hub_test('admin marketplace page primary labels use i18n helper', function (): void {
    ...
    foreach ([
        "hub_i18n_text('HubPack 套件')",
        "hub_i18n_text('本機 HubPack 安裝目錄')",
        "hub_i18n_text('套件名稱')",
        "hub_i18n_text('安裝為服務')",
        "hub_i18n_text('查看 API 文件')",
    ] as $needle) {
        hub_test_assert(str_contains($marketplace, $needle), 'marketplace page label must call hub_i18n_text(): ' . $needle);
    }
});

hub_test('customer portal and playground primary labels use i18n helper', function (): void {
    $pages = [
        'admin/my_services.php' => ["hub_i18n_text('我的服務')", "hub_i18n_text('到 API 測試場')", "hub_i18n_text('API 文件')"],
        'admin/my_tokens.php' => ["hub_i18n_text('我的 Token')", "hub_i18n_text('建立 Token')", "hub_i18n_text('Token 列表')"],
        'admin/my_ip_whitelist.php' => ["hub_i18n_text('IP 白名單')", "hub_i18n_text('選擇 Token')", "hub_i18n_text('目前規則')"],
        'admin/my_usage.php' => ["hub_i18n_text('用量統計')", "hub_i18n_text('目前尚無用量紀錄。')"],
        'admin/change_password.php' => ["hub_i18n_text('變更密碼')", "hub_i18n_text('目前密碼')", "hub_i18n_text('更新密碼')"],
        'admin/playground.php' => ["hub_i18n_text('API 測試場')", "hub_i18n_text('選擇服務')", "hub_i18n_text('請求')", "hub_i18n_text('回應結果')", "hub_i18n_text('介接範例')"],
    ];

    foreach ($pages as $relativePath => $needles) {
        $source = (string)file_get_contents(HUB_ROOT . '/' . $relativePath);
        foreach ($needles as $needle) {
            hub_test_assert(str_contains($source, $needle), $relativePath . ' label must call hub_i18n_text(): ' . $needle);
        }
    }
});
