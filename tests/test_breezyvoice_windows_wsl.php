...
hub_test('BreezyVoice Pascal CUDA 11.8 image can host the managed health API and isolated runner', function (): void {
    ...
    hub_test_assert(
        ...
        && str_contains($dockerfile, "download_model('/opt/breezyvoice/G2PWModel')")
        && str_contains($dockerfile, 'HF_HUB_OFFLINE=1')
        && str_contains($requirements, 'HyperPyYAML==1.2.3')
        && str_contains($requirements, 'ruamel.yaml==0.18.10')
        && str_contains($requirements, 'g2pw==0.1.1')
        && str_contains($requirements, 'onnxruntime-gpu==1.16.0')
        && !str_contains($requirements, 'gradio')
        && !str_contains($requirements, 'deepspeed')
