<?php
declare(strict_types=1);

return [
    __DIR__ . '/../test_command_queue.php',
    __DIR__ . '/../test_cluster_admin.php',
    __DIR__ . '/../test_cluster_router.php',
    __DIR__ . '/../test_environment_probe.php',
    __DIR__ . '/../test_gateway.php',
    __DIR__ . '/../test_hardware_preflight.php',
    __DIR__ . '/../test_model_registry.php',
    __DIR__ . '/../test_phase_auth1.php',
    __DIR__ . '/../test_phase_auth1a2_login_lockout.php',
    __DIR__ . '/../test_phase_auth2a.php',
    __DIR__ . '/../test_phase_dx1.php',
    __DIR__ . '/../test_phase_dx4_client_starter.php',
    __DIR__ . '/../test_phase_p1.php',
    __DIR__ . '/../test_phase_ui1.php',
    __DIR__ . '/../test_phase_ui2.php',
    __DIR__ . '/../test_phase_ui3.php',
    __DIR__ . '/../test_phase_ui4.php',
    __DIR__ . '/../test_phase_ui5.php',
    __DIR__ . '/../test_release_ci.php',
    __DIR__ . '/../test_runtime_portability.php',
    __DIR__ . '/../test_service_settings.php',
    __DIR__ . '/../test_sqlite_safety.php',
    __DIR__ . '/../test_taiwan_address_pack.php',
    __DIR__ . '/../test_windows_core_models_root.php',
];
