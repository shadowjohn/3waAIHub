...
hub_test('Isolated VoxCPM2 jobs retain the exact container runner and managed clone mount', function (): void {
    foreach (['0.1.9', '0.1.7'] as $storedVersion) {
        $db = hub_test_reset_db();
...
hub_test('GPU preflight waits with a stable reason and releases the lease without probing hardware in tests', function (): void {
    ...
    $result = hub_runtime_gpu_preflight($db, $taskId, $run, $lease ?: [], 4096, static fn (): array => [
        'free_vram_mb' => 4096,
        'processes' => [991],
        'process_details' => [[
            'pid' => 991,
    ...
    $task = hub_get_task($db, $taskId);
    $waiting = json_decode((string)($task['waiting_detail_json'] ?? ''), true);
    hub_test_assert(($result['reason'] ?? '') === 'unmanaged_gpu_process'
        && ($result['required_vram_mb'] ?? null) === 4096
        && ($result['free_vram_mb'] ?? null) === 4096
        && ($waiting['gpu_processes'][0]['process_name'] ?? '') === 'ffmpeg'
        && ($waiting['gpu_processes'][0]['used_vram_mb'] ?? null) === 512,
        'unmanaged GPU waits must persist the required, free, process, and VRAM snapshot without killing the process');
});
