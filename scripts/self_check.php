...
foreach ($tables as $table) {
    $db->exec('DELETE FROM ' . hub_sqlite_schema_identifier((string)$table));
}
...
assert(hub_is_safe_absolute_path('/etc') === (hub_platform_id() === 'windows'));
assert(hub_is_safe_absolute_path('/var/lib/docker') === (hub_platform_id() === 'windows'));
...
assert(hub_artifact_safe_path($artifact['path']) === realpath($artifactPath));
