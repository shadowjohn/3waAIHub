...
function hub_execute_command_job(PDO $db, array $job): array
{
    ...
    if ((str_starts_with($action, 'service_') || in_array($action, ['whisper_pascal_ckip_provision', 'manual_vision_provision', 'manual_vision_acceptance', 'paligemma2_provision', 'paligemma2_acceptance', 'breezyvoice_provision'], true)) && !$service) {
    ...
    return match ($action) {
        ...
        'breezyvoice_provision' => hub_run_breezyvoice_provision_job($db, $service, $job),
