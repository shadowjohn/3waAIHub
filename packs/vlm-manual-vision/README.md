...
Provisioning is a private, operator-only root one-shot: it validates the pinned local snapshot before publishing the verified marker. The resident API stays nonroot, never downloads a model, and never receives a provisioning credential.
