...
| --- | --- |
| Service Runtime | 已完成 |
| Local Job Runtime | Preview／薄版 |
| API Gateway | 已完成 |
| Token / IP Control | 已完成 |
| Run History | Preview／薄版 |
| Resource Sampling | Preview／薄版 |
| External Database Profile | 規劃中 |
...
`windows-wsl2-linux-docker` 目前提供固定 distro／`/DATA/3waAIHub-runtime` 的 YOLO Local Job、Web Screenshot CPU Pack Job、Edge TTS CPU Pack Job，以及 Whisper ASR resident Pack Job 四條明確 vertical slice。Whisper 在 Pascal profile 固定走 CUDA 11.8、`small` model、resident queue，stage 與模型快取都留在 WSL ext4；支援基本逐字稿、SRT、VTT 與可選的 `legacy_adaptive_v1` CKIP 字幕重切，`large_v3`、word timestamps、diarization 會在建立 workspace 前明確拒絕。Marketplace 僅在 `whisper-asr` 解析為 ready 的 `pascal-cu118` WSL Runtime 時顯示「準備 CKIP 字幕資產」；它以背景工作將已釘選的 CKIP 資產寫入 `/DATA/3waAIHub-runtime/cache/whisper/ckip`，不會改寫 Windows Control Plane cache，也不會放行 direct `linux-docker`。Web Screenshot、Edge TTS 與 Whisper 僅在 Pack 明確宣告 WSL target、runtime profile ready 時，才會由 PHP task worker 經 `wsl.exe` 呼叫 WSL Docker；其他 UI 或 queue 的 `linux-docker` job 不會自動改送 WSL。Windows 直接選用 `linux-docker` 時，仍會在呼叫 Docker 前以 `exit 78`、`error_code=platform_target_unsupported` 回報不支援。
...
3waAIHub 目前提供薄版 i18n foundation：

- SQLite table：`i18n(id, title, lang, trans)`
- 語系 cookie：`USER_LANG`
- 預設語系：`zh_TW`
- 可選語系：`zh_CN`、`en`、`ja`、`ko`、`es`、`vi`、`th`、`it`
- helper：`__('原字串')`
...
```bash
php scripts/export_i18n_seed.php
```

新安裝或 `php scripts/init_db.php` 會自動匯入 `i18n/seed.json`。匯入只補缺漏，不覆蓋現場已存在的人工修正或重新翻譯內容，因此 repo 可追蹤一份基礎翻譯庫，runtime SQLite 仍保留各站自己的後續調整。
...
```text
http://localhost/3waAIHub/catalog_show/
```

`catalog_show/` 未登入可看功能介紹與介接範例；登入後才可執行 API 測試。若未手動貼 Bearer Token，展示頁會為登入帳號建立本次 session 用的展示 Token，正式 `api.php` Bearer Token 規則不會被放寬。
...
Token IP whitelist 不支援萬用字元寫法，請改用 CIDR：
...
若要讓 Token 不限制來源 IP，最簡單是「不設定任何規則」。`0.0.0.0/0` / `::/0` 適合需要明確留下「全部開放」規則的場景。
...
YOLO 也宣告 Pack Runtime Contract v0.1 的本地 job 模式，方便 NatureWeb 或本機腳本用 workspace 直接呼叫，不必走公開 API：

```bash
cd /DATA/3waAIHub
mkdir -p /DATA/jobs/yolo/predict-001/input
echo '{"image":"input/sample.jpg"}' > /DATA/jobs/yolo/predict-001/request.json

bin/aihub-run yolo_predict \
  --pack yolo \
  --run-id natureweb-yolo-001 \
  --caller natureweb \
  --workspace /DATA/jobs/yolo/predict-001
```

目前宣告的本地 job：

```text
yolo_predict      真實 Ultralytics batch predict runner
yolo_train        真實 Ultralytics training runner
yolo_export_onnx  真實 Ultralytics ONNX export runner
```

`yolo_predict` 最小 workspace contract：
...
`yolo_train` 最小 workspace contract：
...
├─ datasets/
└─ runs/
   ├─ train/output/results.csv
   ├─ train/output/weights/best.pt
...
`yolo_export_onnx` 最小 workspace contract：
...
預設使用 Docker image `3waaihub-yolo-main:0.1.0`；若服務 key 或 image tag 不同，可用 `AIHUB_YOLO_IMAGE=...` 覆寫。Runner 會把 `AIHUB_YOLO_MODELS_DIR` 掛成 container 的 `/models/yolo`，預設 `/DATA/models/yolo`；request 只能指定模型檔名或 workspace-relative path，不接受 host 絕對路徑。

目前主機已保留訓練底模型：`yolo11n.pt`、`yolo26n.pt`、`yolo26s.pt`、`yolo26m.pt`。物種辨識建議先用 `yolo26s.pt` 當正式 baseline，`yolo26n.pt` 當快速流程 smoke，`yolo26m.pt` 留給資料量足夠後的精度測試。
...
### YOLO Model Registry / GPU Warm Pool 1B

`yolo-serving` 是 YOLO Detect `.pt` 的 model registry + serving Pack，用來把外部訓練成果註冊成 Hub 可治理的 `model_ref`，再透過 API 做單張圖片 predict。Phase 1B 增加固定 `yolo-gpu0` 雙槽 warm pool；CPU serving 仍保留作為安全 fallback。
...
```bash
curl -X POST "http://localhost/3waAIHub/api.php?mode=yolo_model_assign_gpu" \
  -H "Authorization: Bearer <TOKEN>" \
  -F "model_ref=yolo:natureweb:training-result-47:v1" \
  -F "slot_no=1"
```
...
```bash
curl -X POST "http://localhost/3waAIHub/api.php?mode=yolo_model_unassign_gpu" \
  -H "Authorization: Bearer <TOKEN>" \
  -F "model_ref=yolo:natureweb:training-result-47:v1"
```
...
- `auto`：GPU hot slot 可用時走 `yolo-gpu0`，否則安全退回 `yolo-cpu`
- `cpu_only`：固定走 CPU
- `gpu_only`：GPU slot 未 hot 時回 `gpu_not_ready`，不退回 CPU

`yolo_predict` 會由 Gateway 將 `model_ref` 轉成 container path，runtime response 需回 `model_ref`、`version_id` / `model_version_id`、`device_used`、`fallback_reason` 與 `detections`。可選 `conf` / `iou` / `imgsz` / `max_det`；`imgsz` 未傳時優先使用 registry metadata，否則 640。Client 不可傳 `host_path`、`model_path`、`artifact_path`、`slot_no`、`device` 或任意 server path。
...
- 預設 `BIOCLIP_REAL_INFERENCE=1`，使用 OpenCLIP 載入 `hf-hub:imageomics/bioclip-2` 跑 zero-shot classification。
- `candidate_labels` 可傳逗號分隔文字或 JSON array。
- 預設 `BIOCLIP_DEVICE=cpu`，適合常駐且不佔 VRAM；需要實驗 GPU 時再手動改成 `cuda`。
- `GET /health` 回 `runtime_level=L5-benchmark-ready`、dependency、device 與 storage 狀態。
...
本階段不接 NatureID 既有 BioCLIP server、不做 taxonomy DB、不做批次分類，也不預設要求 Docker GPU runtime。
...
- 成功回應會是 `mock=false`，並包含有效 device、compute type 與 fallback 狀態；所有候選裝置失敗時回 `503 real_inference_failed`，不會回 mock transcription。
- image 安裝 CUDA 12 cuBLAS 與 cuDNN 9 後，以 `ldconfig` 註冊套件 library path；CPU-only Docker host 請將服務設定 `USE_GPU=0`，重建 compose 後仍可使用 CPU fallback。
...
- `profile_status` 可向已驗證的 Profile member 回傳 unconfirmed `prompt_text` ASR draft
