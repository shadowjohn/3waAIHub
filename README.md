...
`structure-ppstructurev3` 目前是 L4 `real-inference` document parsing Pack：
...
- `GET /health` 回 `runtime_level=L4-real-inference`、dependency 與 storage 狀態。
...
本階段不做 L5 benchmark-ready、不做 GPU tuning、不做 viewer、不做批次管理 UI。

### docparser Runtime Level

`docparser` is PhaseDoc-1B `L4-real-service-readiness`.
